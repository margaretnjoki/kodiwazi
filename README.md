...
🔗 **Live now:** [https://kodiwazi-api.fly.dev/api/v1/swagger-ui/index.html](https://kodiwazi-api.fly.dev/api/v1/swagger-ui/index.html)

> KodiWazi is an API, not a website. There's no visual homepage. The links below either return raw JSON (correct, expected behavior for an API) or open the interactive Swagger UI, which is the human-friendly way to explore it.
...
- **Contributor:** a lightweight account (email and password), just enough to distinguish one person's submissions from another's and support abuse resistance rules.
- **RentSubmission:** one person's report: "I pay `amount` for a `HouseType` in this `Area`, with or without utilities included." Carries a `status` (`ACTIVE`, `FLAGGED`, or `REMOVED`) so abuse resistance logic can act without destroying data.
...
1. `POST /auth/register`: create a Contributor account (email and password; password is BCrypt hashed).
...
Full interactive documentation, with request/response schemas and a "try it out" button, is available at the [Swagger UI link above](https://kodiwazi-api.fly.dev/api/v1/swagger-ui/index.html). Summary below.

> **Note on Swagger UI and enums:** Swagger UI renders `houseType` as free text JSON in request bodies (a known limitation; it only shows dropdowns for query parameters, not body fields). Use the exact values listed below, spelled and cased precisely, or the request will fail validation. Clicking **Schema** (next to **Edit Value** on any endpoint) also shows the accepted values.
...
| Method | Endpoint | Auth | Description |
|---|---|---|---|
| GET | `/areas` | Public | List areas. Optional query params: `regionId`, `regionName` (case insensitive, e.g. `nairobi`), `name` (case insensitive partial match on area name, e.g. `south` matches "South B"/"South C") |

### HouseType, valid values

Use exactly one of these strings (case sensitive) anywhere `houseType` is required, in a path, query, or request body:
...
Every estimate carries a confidence score (0 to 100) and label (`LOW`, `MEDIUM`, or `HIGH`), combining three factors:
...
3. **Consistency:** how tightly submitted amounts agree with each other, measured via coefficient of variation (standard deviation divided by mean). Tightly clustered reports read as more trustworthy than wildly scattered ones.
...
- **One active submission per contributor per segment.** If a contributor submits again for a segment they've already reported on (same Area, HouseType, and utilities status), their existing submission is updated in place rather than a duplicate being created. This ensures no single contributor can inflate their influence on a segment's estimate simply by resubmitting.
